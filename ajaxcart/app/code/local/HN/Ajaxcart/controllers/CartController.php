 ...
 */
class HN_Ajaxcart_CartController extends Mage_Core_Controller_Front_Action
{
	/**
	 ...
	 *Add multi items in the shopping cart
	 */
	public function addmultiAction() {
		try {
			for ($i = 0; $i < 10 ; $i++) {
				$product_param_name = 'product'.$i;
				$product_id = $this->getRequest()->getParam($product_param_name);
				if ($product_id != '') {
					$param = array();
					$qty_pr = 'qty'.$i;
					$params['qty'] = $this->getRequest()->getParam($qty_pr);
					$cart   = $this->_getCart();
					$product = Mage::getModel('catalog/product')
					->setStoreId(Mage::app()->getStore()->getId())
					->load($product_id);
